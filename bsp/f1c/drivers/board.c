...
#include "mmu.h"
#include "lvgl.h"
volatile uint32_t AutoIncData=0;
...
static void mmu_init(void)
{
    struct mem_desc r6_mem_desc[] =
    {
        ...
        { 0x80000000, 0x81800000 - 1, 0x80000000, RW_CB },   /* 32M cached SDRAM memory */
        //{ 0x80000000, 0x82000000 - 1, 0x81800000, RW_NCNB },   /* 32M cached SDRAM memory */
...
static inline void SSdelay(int loops)
{
...
void rt_hw_board_init(void)
{
	...
	video_set_pll_clk(297000000);
	cpu_set_clk(600000000);
	//bus_gate_clk_enalbe(LCD_GATING);
	dram_gate_clk_enable(BE_GATING_DRAM);
	//dram_gate_clk_enable(FE_GATING_DRAM);
