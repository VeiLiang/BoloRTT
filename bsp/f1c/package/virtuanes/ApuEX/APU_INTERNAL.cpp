...
INT	APU_INTERNAL::decay_lut[16] = {0};
...
APU_INTERNAL::APU_INTERNAL()
{
	...
	// 仮設定
	cycle_rate = (INT)(cpu_clock*65536.0f/44100.0f);
...
//	ZEROMEMORY( &ch4, sizeof(ch4) );
	...
	reg4015 = sync_reg4015 = 0;
	int num_sample = nRate/60;
	DEBUGOUT( "num_sample:%d \n",num_sample );
	for(int i=0; i< 32 ; i++)
	{
		vbl_lut[i] = vbl_length[i]*num_sample;
	}
	for(int i=0; i< 16 ; i++)
	{
		decay_lut[i] = num_sample * (i + 1);
	}
	// Sweep complement
	ch0.complement = 0x00;
	ch1.complement = 0xFF;
...
void	APU_INTERNAL::Setup( FLOAT fClock, INT nRate )
{
	...
	cycle_rate = 2470762;//(INT)(fClock*65536.0f/(float)nRate);
...
INT	APU_INTERNAL::Process( INT channel )
{
	switch( channel ) {
		case	0:
			return	RenderRectangle( ch0 ,TRUE);
		case	1:
			return	RenderRectangle( ch1 ,FALSE);
		case	2:
...
void	APU_INTERNAL::Write( WORD addr, BYTE data )
{
	switch( addr ) 
	{
		...
		case	0x4002:	case	0x4003:
			WriteRectangle( 0, addr, data );
			break;
		case	0x4004:	case	0x4005:
		case	0x4006:	case	0x4007:
			WriteRectangle( 1, addr, data );
			break;
...
#ifdef USE_FAST_APU
			ch.env_decay = decay_lut[(data&0x0F)];
#else
...
// Render Rectangle
INT	APU_INTERNAL::RenderRectangle( RECTANGLE& ch ,INT is_ch0)
{
	if( !ch.enable || ch.len_count <= 0 )
		return	0;
	...
	if( !(Config.sound.bChangeTone && ChannelTone[(!ch.complement)?0:1][ch.reg[0]>>6]) ) 
	{
		// 插??理
		long long 	total;
		long	sample_weight = ch.phaseacc;
		...
		return volume*total/cycle_rate;
...
#else
	if(ch.holdnote == FALSE && ch.len_count)
	{
		ch.len_count--;
	}
	ch.env_phase -= 4;
...
//    */
//    if (ch.freq < 8 || (FALSE == ch.swp_inc && ch.freq > ch.freqlimit))
//       return	0;
	/* frequency sweeping at a rate of (sweep_delay + 1) / 120 secs */
   if (ch.swp_on && ch.swp_shift)
   {
      ch.swp_phase -= 2; /* 120/60 */
      while (ch.swp_phase < 0)
      {
         ch.swp_phase += ch.swp_decay;

         if (ch.swp_inc) /* ramp up */
         {
            if (TRUE == is_ch0)
               ch.freq += ~(ch.freq >> ch.swp_shift);
            else
	...
	// DEBUGOUT( "cycle_rate:%d \n",cycle_rate );
   ch.phaseacc -= cycle_rate*2; /* # of cycles per sample */
   if (ch.phaseacc >= 0)
      return 0;
   ...
   int output = 0;
   if (ch.env_fixed)
      output = ch.volume << 8; /* fixed volume */
   else
      output = (ch.env_vol ^ 0x0F) << 8;

   while (ch.phaseacc < 0)
   {
      ch.phaseacc += INT2FIX(ch.freq + 1);
      ch.adder = (ch.adder + 1) & 0x0F;
