...
#define	ZPWRW(A,V)	{ RAM[(BYTE)((A)+0)]=(V)&0xff;RAM[(BYTE)((A)+1)]=((V)>>8)&0xff; }
...
#if 1 ///arm 默???了非???? 影?效率 所以要从以下方式??
	WORD	ret;
	ret  = (WORD)RAM[(addr+0)&0x07FF];
	ret |= (WORD)RAM[(addr+1)&0x07FF]<<8;
#else
