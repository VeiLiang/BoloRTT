...
int rt_usb_hcd_int_xfer(uhcd_t  hcd,
                                  upipe_t pipe,
                                  void   *buffer,
                                  int     nbytes,
                                  int     timeout)
{
    ...
    return rt_usb_hcd_pipe_xfer(hcd, pipe, buffer,nbytes, timeout);
}
int rt_usb_hcd_control_xfer(uhcd_t hcd, uinst_t uinst, ureq_t setup,
    void* buffer, int nbytes, int timeout)
{
	int len = 0;
	if(setup->request_type & USB_REQ_TYPE_DIR_IN)
	{
		return hcd->ops->pipe_xfer(uinst->pipe_ep0_in, USBH_PID_SETUP, buffer, nbytes, timeout);
		/* do control transfer request */
		if(rt_usb_hcd_setup_xfer(hcd, uinst->pipe_ep0_out, setup, timeout) != 8)
		{
			return -RT_EIO;
		}
		len = rt_usb_hcd_pipe_xfer(hcd, uinst->pipe_ep0_in, setup,setup->wLength, timeout);
		if(rt_usb_hcd_pipe_xfer(hcd, uinst->pipe_ep0_out, RT_NULL, 0, timeout) != 0)
		{
			return -RT_EIO;
		}
		return len;
	...
	else
	{
		...
		if(rt_usb_hcd_pipe_xfer(hcd, uinst->pipe_ep0_in, RT_NULL, 0, timeout) != 0)
		{
			return -RT_EIO;
		}
	}
...
static void rt_usbh_hid_callback(void* context)
{
    ...
    /* parameter check */
     RT_ASSERT(pipe->inst->intf[0]->device->hcd != RT_NULL);

//    rt_usb_hcd_int_xfer(pipe->inst->intf[0]->device->hcd, pipe, hid->buffer,
...
//            intf, ep_desc, rt_usbh_hid_callback);
        ...
        hid->pipe_in->user_data = hid; // use for callback in indicate which interface
    ...
    pipe = hid->pipe_in;
    RT_DEBUG_LOG(RT_DEBUG_USB,
                     ("initialize hid protocal done!\n"));
    /* parameter check */
     RT_ASSERT(pipe->inst->intf[0]->device->hcd != RT_NULL);
    
    rt_usb_hcd_int_xfer(pipe->inst->intf[0]->device->hcd, hid->pipe_in,
